...
class PlotPoint:
    def __init__(self, tick, value, name, color, marker = 'point'):
        self.tick = tick
        self.value = value
        self.name = name
        self.color = color
    ...
    def _plot_strategy(self):
        ...
        for lines, points in zip(lines_list, points_list):
            ...
            for line_name, line in lines.items():
                if line_name.lower().find('bound')>=0:
                    plt.fill_between(np.arange(0, len(line[0])),line[0], line[1], color = line[2], label=line_name)
                else:
                    plt.plot(line[0], label=line_name, color = line[1])
            ...
            for point_name in point_names:
                plt.scatter(
                    ...
                    c = [point.color for point in points if point.name == point_name][0],
                    marker = [point.marker for point in points if point.name == point_name][0],
                    label = point_name)
